...
export default function Hotels() {
	...
	async function load() {
		...
		setError(null);
		try {
			const data = await getAll('hotels');
			setItems(Array.isArray(data) ? data : []);
		} catch (e) {
			setError(e?.error || 'Failed to load hotels');
			setItems([]);
		} finally {
			setLoading(false);
		}
	...
	return (
		<section className="space-y-6">
			<div className="flex items-center justify-between">
				...
				<button className="btn btn-primary" onClick={() => { setEditItem(null); setFormOpen(true); }}>Add Hotel</button>
			</div>
				...
				<DataList
					...
					renderItem={(h) => (
						<ItemCard
							...
							title={h.name}
							subtitle={h.address}
							meta={[`Rating: ${h.rating}`, `Price/Night: ${h.price_per_night}`, `Source: ${h.source}`]}
							onEdit={() => startEdit(h)}
...
function HotelForm({ initial, onClose, onSaved }) {
	const [name, setName] = useState(initial?.name || '');
	const [pricePerNight, setPricePerNight] = useState(initial?.price_per_night ?? '');
	...
	async function onSubmit(e) {
		...
		setErr(null);
		try {
			const payload = { name, price_per_night: Number(pricePerNight), rating: Number(rating), address, source };
			if (isEdit) {
				await updateItem('hotels', initial.id, payload);
			} else {
	...
	return (
		<div className="fixed inset-0 bg-black/30 flex items-center justify-center p-4 z-50">
			<form onSubmit={onSubmit} className="card w-full max-w-lg p-5 space-y-4 bg-white">
				<h3 className="text-lg font-semibold">{isEdit ? 'Edit' : 'Add'} Hotel</h3>
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
					<div>
						...
						<input type="number" className="input" value={pricePerNight} onChange={e => setPricePerNight(e.target.value)} required />
					</div>
					<div>
